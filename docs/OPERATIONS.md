...
- **测试环境已拆除。** 容器、网络、三个卷（`server-test_pgdata` / `_redisdata` /
  `_musiccache`）、目录 `/opt/michael-ide-deploy/server-test` 和部署锁全部删除。拆之前确认
  过它的库是空的（0 用户 / 0 密钥）。下面第一节写的两套环境**仍然可用**——`TARGET=test`
  那条路一行代码都没动，需要时按原样再拉起来一套即可，这份文档就是为了让重建不用重新推导。
- **API key 存储迁移第一步已上生产。** 迁移 `20260844` 应用成功，13 个 key 全部完成
  哈希 + 密文回填（`待回填 = 0`），171 个用户无感。**第二步（清除明文）还没做**——
  见下面「第二次部署：清除明文」，那一步需要有人明确决定什么时候做。
...
### 第一次部署：加列 + 回填（用户完全无感）—— **已于 2026-08-19 完成**

生产实测结果（这就是下面那条验收 SQL 跑出来的）：
...
「仍有明文 13」是**这一步的正常状态**，不是没做完：明文列是回滚用的兜底，第二步才清。
...
- **SSH 握手会随机掉**，报的是 `banner exchange ... invalid format`。这不是密钥问题，重试就行。
  但要小心它伪装成"远端状态"：`.env` 存在性检查原来用 `ssh_run`，而 ssh 的 **255** 表示
  **它自己**失败、不是远端命令的返回值。2026-08-19 实拍：连掉三次 → 脚本打印
  「No .env exists on the server. Copy .env.example to .env」并中止。那条提示是错的，
  而且照着做会把生产的密码和密钥抹掉。已改成 `ssh_true`：只有远端真的回答了才返回真值，
  连不上就说连不上、什么都不判断。**新写远端判断时用 `ssh_true`，别用 `ssh_run`。**
