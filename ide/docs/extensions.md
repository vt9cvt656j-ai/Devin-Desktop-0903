# Writing a Devin IDE extension

Devin IDE has its own lightweight extension format. It is **not** the VS Code
`.vsix` format — Devin IDE embeds Monaco (VS Code's editor component) but not the
...
- **From the bundled registry:** Extensions panel → **Available** → **Install**.
  Devin IDE ships sample extensions (Word Count, Insert Date) compiled into the
  app.
