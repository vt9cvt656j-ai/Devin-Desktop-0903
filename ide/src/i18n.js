const EN = {
  ...
  "assistant.configFirst": "Configure an AI provider first",

  "statusbar.commands": "Commands",

  "settings.title": "AI provider",
  "settings.sub": "Any OpenAI-compatible endpoint. Stored locally on this machine.",
  ...
  "settings.configure": "Configure provider…",
...
const ZH_CN = {
  ...
  "assistant.configFirst": "请先配置 AI 提供商",
  "statusbar.commands": "命令",
  "settings.title": "AI 提供商",
  "settings.sub": "支持任何 OpenAI 兼容的 API 端点。密钥仅存储在本机。",
...
let translations = { en: EN, "zh-CN": ZH_CN };
let changeListeners = [];
...
export function registerLocale(locale, dict) {
  translations[locale] = { ...EN, ...dict };
}

export function setLocale(locale) {
  if (!translations[locale]) return;
  currentLocale = locale;
  localStorage.setItem("michael-ide-locale", locale);
  applyToDOM();
