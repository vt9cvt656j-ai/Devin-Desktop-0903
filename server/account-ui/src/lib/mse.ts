...
async function attempt(
   ...
   *
   * 判据是「这份明文里有没有我们会**当成功数据信**的东西」。只有明文 **2xx** 才拒绝：
   *   - 明文 2xx：应用会把 body 当成功结果信（伪造的 `{"role":"admin"}` 正是这种）。拒绝。
   *   - 加密流：每帧各带 GCM tag，伪造帧解不开，交给流解码器去炸即可
   *   - 3xx / 101：body 不是数据（重定向浏览器自己跟，101 是协议升级）
   *   - 4xx / 5xx：**基础设施的错误**（nginx 限速 429、请求过大 413、Cloudflare 502
   *     / 挑战页……）。这些是明文很正常，不是「攻击者塞了假成功数据」——应用只会把它
   *     当一次失败。以前把 4xx 也当降级拒绝，结果是：用户多试几次触发登录限速的 429，
   *     加密客户端直接抛「downgrade」，登录页显示「网络异常」，把人锁在门外。放行它们，
   *     让上层看到真实状态码（429 就显示「稍后再试」），而不是一个密码学硬错误。
   *     伪造一个明文 4xx 最多是拒绝服务或改改错误文案，拿不到任何权限或数据。
   */
  const sealedStream = ct.includes("text/event-stream") && res.headers.get("x-mse-stream") === "1";
  // 只有明文 2xx 触发降级判定；3xx/4xx/5xx/101 一律放行（它们不是「可信的成功数据」）。
  const trustworthyPlaintext = res.status >= 200 && res.status < 300;
  if (!sealedStream && trustworthyPlaintext) {
