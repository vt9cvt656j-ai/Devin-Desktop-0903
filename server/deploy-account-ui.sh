#!/usr/bin/env bash
# Publish the account console (code.mrday.one/dashboard and /billing) to the server.
#
#   SERVER_KEY=~/.ssh/michael_server ./deploy-account-ui.sh
#
# deploy.sh next to this file ships the backend containers; it does not touch the
# frontends. This script is the missing half for account-ui.
#
...
WEB_ROOT="${WEB_ROOT:-/var/www/michael-account}"
STAGE_DIR="${STAGE_DIR:-/root/account-deploy}"
