...
# 改写「流量指向哪个后端端口」那一行，然后校验 + 优雅 reload。
#
# 这个文件**故意没有仓库副本**：它是生成物，内容取决于此刻哪种颜色在服务。仓库里放一份
# 「参考副本」的话，任何会安装 nginx 配置的东西（比如 install-nginx.sh）都会把它按字面
# 覆盖回蓝色端口 —— 而绿色正在服务时，那一下就是把 nginx 指向一个已经停掉的端口，全站 502。
# 首次安装由这个函数自己创建：文件不存在时 live_port 返回空，调用方按蓝色处理。
#
# 只有一个 server：同一时刻只有一种颜色收流量。不做双活是刻意的 —— 网关有进程内状态
# （线路冷却、卡死记号都是 per-process 的 HashMap），双活会让两个副本各学各的。
