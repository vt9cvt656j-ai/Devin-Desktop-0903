...
export async function load(url, ctx, next) {
  ...
  if (/\/src\/main\.js$/.test(url) && r.source) {
    ...
    s += "\n\n/* __E2E_HOOK__ */\nif (globalThis.__E2E__) { globalThis.__testHooks = { _executeToolStepInner, backend, _normalizeFsPath, _resolveRel, setMode: (m)=>{ _currentAiMode = m; }, getMode: ()=>_currentAiMode };\n  try { globalThis.__testHooks.extra = { showToast, _fsDelta: typeof _fsDelta }; } catch (e) { globalThis.__testHookErr = e.message; } }\n";
    return { ...r, source: s };
  }
  return r;
}
