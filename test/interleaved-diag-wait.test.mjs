...
test("秒回的 TS 目标不许替慢的 rust/python 提前结束等待", async () => {
  const targets = [
    { rel: "src/app.ts", isTs: true, model: { uri: "ts" } },
    { rel: "src/engine.rs", isTs: false, model: { uri: "rs" } },
  ];
...
test("慢目标一直不出结果时，各自超时，不把整批拖满总闸", async () => {
  const targets = [
    { rel: "a.ts", isTs: true, model: { uri: "ts" } },
    { rel: "b.rs", isTs: false, model: { uri: "rs" } },
  ];
...
test("TS 目标用的是它自己那个短期限，不是总闸", async () => {
  const targets = [{ rel: "a.ts", isTs: true, model: { uri: "ts" } }];
